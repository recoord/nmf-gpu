...
void update_div(Matrix W0, Matrix H0, Matrix X0, const float thresh, const uint32_t max_iter, cudaStream_t stream);
...
int32_t main(int32_t argc, char *argv[]) {
    ...
    update_div(W, H, X, CONVERGE_THRESH, MAX_ITER, stream);
...
void update_div(Matrix W0, Matrix H0, Matrix X0, const float thresh, const uint32_t max_iter, cudaStream_t stream) {
    ...
    Matrix Z(0.0f, M_padded, N_padded, stream);

    // Matrix to hold W'*Z
    Matrix WtZ(0.0f, K_padded, N_padded, stream);

    // Matrix to hold Z*H'
    Matrix ZHt(0.0f, M_padded, K_padded, stream);

    // Matrix to hold sum(W) [sum of cols of W]
    Matrix sumW(0.0f, 1, K_padded, stream);

    // Matrix to hold sum(H,2) [sum of rows of H]
    Matrix sumH2(0.0f, K_padded, 1, stream);

    // matrices to hold padded versions of matrices
    Matrix W(0.0f, M_padded, K_padded, stream);
    Matrix H(0.0f, K_padded, N_padded, stream);
    Matrix X(0.0f, M_padded, N_padded, stream);
    ...
    W0.clone_to_padded(&W, stream);
    H0.clone_to_padded(&H, stream);
    X0.clone_to_padded(&X, stream);
