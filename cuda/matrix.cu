...
Matrix::Matrix(uint32_t rows, uint32_t cols, cudaStream_t stream) {
    this->rows = rows;
    this->cols = cols;
...
Matrix::Matrix(float *host_data, uint32_t rows, uint32_t cols, cudaStream_t stream) {
    ...
    cudaAssert(cudaMemcpyAsync(this->data, host_data, size, cudaMemcpyHostToDevice, stream));
    cudaAssert(cudaStreamSynchronize(stream));
}
...
//     cudaError_t err;
//     err = cudaMemcpy2D(
//         Apad.data, sizeof(float) * M_padded, A.data, sizeof(float) * M, sizeof(float) * M, N,
...
void Matrix::clone_to_padded(Matrix* padded, cudaStream_t stream) {
    if(this->rows > padded->rows) {
        fprintf(stderr, "copy_to_padded: padded number of rows must be >= original\n");
        exit(1);
    }
    if(this->cols > padded->cols) {
        fprintf(stderr, "copy_to_padded: padded number of cols must be >= original\n");
        exit(1);
    }

    // data: 0x7b2503c00000
    // padded: 0x7b2509800000
    // copy_to_padded: 4096 128 4096 128

    printf("data: %p\n", this->data);
    printf("padded: %p\n", padded->data);
    printf(
        "copy_to_padded: %i %i %i %i\n", this->rows, this->cols, padded->rows, padded->cols
    );

    cudaAssert(cudaMemcpy2DAsync(
        padded->data, 128 * sizeof(float), this->data, 128 * sizeof(float), 128 * sizeof(float),
        128, cudaMemcpyDeviceToDevice, stream
    ));

    // cudaAssert(cudaMemcpy2DAsync(
    //     padded->data, padded->cols * sizeof(float), this->data, this->cols * sizeof(float), this->cols * sizeof(float),
    //     this->rows, cudaMemcpyDeviceToDevice, stream
    // ));
}
